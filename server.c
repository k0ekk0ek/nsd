	...
	int cmdsocket)
{
	...
	{
		xfrd_xfr_state_type state = xfrd_xfr_ok;
		zone = (zone_type *)node->elem;
		if(zone->is_updated) {
			if(zone->is_bad) {
				nsd->mode = NSD_RELOAD_FAILED;
				state = xfrd_xfr_invalid;
			} else {
				state = xfrd_xfr_ok;
			}
			task_new_soainfo(
				nsd->task[nsd->mytask], &last_task, zone, 0U, state);
			zone->is_updated = 0;
		}
	}
...
void server_verify(struct nsd *nsd, int cmdsocket)
{
	      ...
	      nsd->verifier_count < nsd->verifier_limit)
	{
		...
		nsd->next_zone_to_verify = verify_next_zone(nsd, zone);
	}
