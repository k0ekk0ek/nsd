		...
		nsd_type* nsd, zone_type* zone)
{
	...
	switch(pid) {
		...
		case  0: if(writefd) {
			...
			/* FIXME: use VERIFIER_FEED_ZONE_ defines here */
			if(zone->opts->pattern->verifier_feed_zone == 1 ||
			  (zone->opts->pattern->verifier_feed_zone == 2 &&
		...
		netio_event_types_type event_types)
{
	...
	if(rr) {
		print_rr(data->to_stdin_f, data->state, rr, data->region, data->buffer);
	} else {
	...
	if(zone->opts->verifier_feed_zone == 1 ||
	  (zone->opts->verifier_feed_zone == 2 &&
	   nsd->options->verifier_feed_zone == 1)) {
		...
		v->to_stdin_user_data.region = s->region;
		v->to_stdin_user_data.buffer = buffer_create(s->region, MAX_RDLENGTH);
